...
export async function GET(request: NextRequest){
  ...
  const positionOffsets = generateGridArray(20, 7, "1.5cm", "8cm", "0.1cm", "0.1cm")
  ...
  const LidPDF = ({lid: inlid, position}: {lid:Lede, position:number} ) => {
    ...
    <View style={{position:"absolute", ...positionOffsets[position], borderRight:"1pt solid grey"} } wrap={false} debug={false}>
      ...
      <Text debug={txtdbg} style={{...commmonTextStyles,bottom:"0.5cm",left: "1.5cm", width: "2.5cm", textAlign:"left"}} hyphenationCallback={w => [w]}>{lid.noemnaam}</Text>
      ...
      <Text debug={txtdbg} style={{...commmonTextStyles,bottom:"0.1cm"/*4.5*/,left: "4.95cm" ,fontSize:"0.3rem", maxHeight: "6mm", width: "3cm", textAlign:"left" }} hyphenationCallback={w => [w]}>{lid.allergiee}</Text>
      ...
      <QRCodeSVGPDF style={{position:"absolute",top:"0.0cm",left: "0.0cm"}} width={"15mm"} height={"15mm"} marginSize={4} value={`${lid.id}`}/>
    </View>
  )}
  ...
  const OutDoc = ({lede, startPos = 0}:{ lede:Lede[], startPos?:number}) => (
    <Document style={{fontFamily:"OpenSans", fontWeight: "medium"}}>
      { chunk(lede.filter(ledefilter).toSpliced(0,0,...blanklede(startPos)),140).map((pagelede,idx) => (
