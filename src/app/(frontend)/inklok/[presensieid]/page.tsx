...
export default async function Inklok({ params: paramsPromise }: Args) {
  ...
  async function processInklokWithNotes(formData: FormData) {
    "use server"
    ...
    if ("error" in inklokres) {
      throw new Error(`Inklok failed: ${inklokres.error}`);
    }
    revalidatePath(`/inklok/${currentPresensieId}`);
  ...
  if (lid) {
    if (presensie.notes_required) {
      return (
        <div className="p-4">
          <h1 className="text-2xl font-bold mb-4">Notas Vereis vir {presensie.naam}</h1>
          <form action={processInklokWithNotes}>
